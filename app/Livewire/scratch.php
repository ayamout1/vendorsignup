...
use App\Models\{Vendor, Address, AgreementForm, Capability, Equipment, Insurance, ServiceFee, W9Submission, User};
...
class MultiStepForm extends Component
{
    ...
    public $addresses, $vendor_name, $owner_name, $owner_phone, $vendor_type, $vendor_phone, $vendor_email, $vendor_fax, $vendor_website;
    ...
    public $file_path, $is_certified, $signature_path, $name, $title;
    ...
    {
        $this->addresses = [['address' => '', 'address2' => '', 'city' => '', 'state' => '', 'postal' => '', 'country' => '', 'address_type' => 'billing']];
        $this->step = 1;
    ...
    {
        try {
            $this->vendorId = Str::uuid();
            // Create vendor in Laravel DB
            $vendor = $this->createVendorInLaravelDB();
            // Create vendor in SuiteCRM DB
            $this->createVendorInSuiteCRMDB($vendor);
        } catch (\Exception $e) {
            Log::error('Error in submitToSuiteCRM: ' . $e->getMessage());
        }
    }

    private function createVendorInLaravelDB()
    {
        // Create and return the Vendor model
        $user = User::create([
    ...
    private function createVendorInSuiteCRMDB($vendor)
    {
        // Insert vendor data into SuiteCRM's vsf_vendornetwork table
        // Handle file uploads and update file paths in SuiteCRM
        // Insert address data into SuiteCRM's vsf_addressnew table
        // Insert relationship data into SuiteCRM's relationship table
          // Insert vendor data into SuiteCRM's vsf_vendornetwork table
...
DB::connection('suitecrm')->table('vsf_vendornetwork')->where('id', $this->vendorId)->update([
    ...
    // Utility methods like resetForm, addAddress, removeAddress, generateFileName, etc.    public function resetForm()
